...
/**
 * Cumulative declining-balance schedule generator with broken-period interest on the first installment —
 * a common NBFC lending pattern.
 *
 * <p>
 * The loan keeps a plain-annuity EMI (which the stock cumulative generator already produces under
 * {@code SAME_AS_REPAYMENT_PERIOD}) but the <b>first</b> installment's interest is charged on the actual number of
 * days between disbursement and the first repayment (the "broken period"), at a 30-day-month daily rate, rather than
 * a full month. When the broken period exceeds 30 days the excess interest is collected as a surcharge on top of the
 * regular EMI (the first installment's principal stays on a 30-day basis).
 * </p>
 *
 * <p>
 * Under {@code SAME_AS_REPAYMENT_PERIOD} the stock first-period interest equals {@code outstandingBalance * monthlyRate}
 * (a full 30-day month). The broken-period interest is {@code outstandingBalance * (monthlyRate / 30) * brokenDays}. So
 * this generator overrides <b>only</b> the first period and leaves every other period (and all
 * non-{@code SAME_AS_REPAYMENT_PERIOD} loans) to the stock behaviour via {@code super}.
 * </p>
 *
 * <p>
 * Registered as {@link Primary} so it transparently replaces the stock cumulative declining-balance generator wherever it
 * is injected (e.g. {@code DefaultLoanScheduleGeneratorFactory}); the gate below makes it a no-op for everything except a
 * broken first period on a {@code SAME_AS_REPAYMENT_PERIOD} product, so other products are unaffected.
 * </p>
 */
@Component
@Primary
public class BrokenPeriodDecliningBalanceLoanScheduleGenerator extends CumulativeDecliningBalanceInterestLoanScheduleGenerator {

    private static final int DAYS_IN_MONTH = 30;
    // Daily rate = annualRate% / 12 (months) / 100 (percent) / 30 (days) = annualRate / 36000.
    private static final BigDecimal DAILY_RATE_DIVISOR = BigDecimal.valueOf(36000);
    // Monthly rate = annualRate% / 12 / 100 = annualRate / 1200 (also the full 30-day-month interest factor).
    private static final BigDecimal MONTHLY_RATE_DIVISOR = BigDecimal.valueOf(1200);

    public BrokenPeriodDecliningBalanceLoanScheduleGenerator(final ScheduledDateGenerator scheduledDateGenerator,
    ...
    public PrincipalInterest calculatePrincipalInterestComponentsForPeriod(final PaymentPeriodsInOneYearCalculator calculator,
        ...
        // Only the FIRST period of a plain-annuity (SAME_AS_REPAYMENT_PERIOD) loan deviates.
        // TODO: replace this config-based gate with an explicit per-product flag when more lenders are onboarded.
        ...
        if (brokenDays == DAYS_IN_MONTH) {
            return stock; // a full 30-day month: stock already matches
        }

        // Broken-period interest = round(OB * dailyRate * brokenDays), dailyRate = annualRate% / 12 / 100 / 30
        ...
        if (brokenDays > DAYS_IN_MONTH) {
            ...
            // The reference implementation computes round(EMI_unrounded - OB*dailyRate*30) using the UNROUNDED
            // annuity EMI; the whole-rupee EMI would round 1 rupee differently here, so derive the unrounded EMI.
            final BigDecimal monthlyRate = loanApplicationTerms.getAnnualNominalInterestRate().divide(MONTHLY_RATE_DIVISOR, mc);
